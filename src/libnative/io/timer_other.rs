...
fn now() -> u64 {
    unsafe {
        let mut now: imp::timeval = intrinsics::init();
...
fn helper(input: libc::c_int, messages: Port<Req>) {
    ...
    let mut timeout: imp::timeval = unsafe { intrinsics::init() };
    ...
    'outer: loop {
        ...
        match unsafe {
            ...
            1 => {
                loop {
                    match messages.try_recv() {
                        ...
                        Some(NewTimer(timer)) => insert(timer),

                        Some(RemoveTimer(id, ack)) => {
                            match dead.pop(&id) {
                                Some(i) => { ack.send(i); continue }
                                None => {}
                            }
                            let i = active.iter().position(|i| i.id == id);
                            ack.send(active.remove(i.expect("no timer found")));
                        }
                        None => break
                    }
                }
            ...
            n => fail!("helper thread failed in select() with error: {}", n)
        }
    }
}
...
mod imp {
    ...
    extern {
        pub fn select(nfds: libc::c_int,
                      ...
                      writefds: *fd_set,
                      errorfds: *fd_set,
                      timeout: *timeval) -> libc::c_int;

        pub fn gettimeofday(timeval: *mut timeval,
